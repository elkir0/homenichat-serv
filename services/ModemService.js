...
const MODEM_PROFILES = {
  ec25: {
    name: 'Quectel EC25',
    slin16: false,  // 8kHz audio (UAC mode)
    msg_storage: 'me',
    disableSMS: false,
    useUAC: true,   // EC25 uses USB Audio Class (NOT serial TTY)
    audioCommands: [
      'AT+QAUDMOD=3',     // USB Audio mode (REQUIRED for VoLTE audio!)
      'AT+QPCMV=1,2',     // Voice over UAC (USB Audio Class)
      'AT+QEEC=1,1,1024', // Echo Cancellation Enhanced
    ],
    portOffset: {
      data: 2,  // ttyUSB2 pour EC25
      audio: 1, // ttyUSB1 (only used as fallback, UAC preferred)
    },
  },
  sim7600: {
    name: 'Simcom SIM7600',
...
`;
  ...
  generateQuectelConf(config = {}) {
    ...
    // Configuration auto-generated by Homenichat
    let confContent = `; Homenichat - Configuration chan_quectel
; Generated automatically - ${new Date().toISOString()}
; EC25: UAC mode (quec_uac=1, alsadev) | SIM7600: TTY mode (audio=ttyUSB)
...
`;
    ...
    // Helper to generate modem section based on type
    const generateModemSection = (modemName, dataPort, modemType, extraConfig = {}) => {
      const type = (modemType || 'ec25').toLowerCase();
      const profile = MODEM_PROFILES[type] || MODEM_PROFILES.ec25;
      let section = `\n[${modemName}]\ndata=${dataPort}\n`;

      if (profile.useUAC) {
        // EC25: USB Audio Class mode (VoLTE)
        // AT+QAUDMOD=3 and AT+QPCMV=1,2 applied at runtime by modem-init/watchdog
        section += `quec_uac=1\nalsadev=plughw:CARD=Android,DEV=0\n`;
      } else {
        // SIM7600: TTY serial audio mode
        const audioPort = extraConfig.audioPort || this.calculateAudioPort(dataPort, type);
        section += `audio=${audioPort}\n`;
        section += `rxgain=-5\ntxgain=-18\n`;
      }

      section += `slin16=${profile.slin16 ? 'yes' : 'no'}\n`;

      // EC25 UAC gains (applied in config, fine-tuned by AT commands at runtime)
      if (profile.useUAC) {
        section += `rxgain=10\ntxgain=10\n`;
      }

      if (extraConfig.imsi) section += `imsi=${extraConfig.imsi}\n`;
      else section += `; imsi auto-detected\n`;

      if (extraConfig.phoneNumber) {
        section += `exten=+${extraConfig.phoneNumber.replace(/^\+/, '')}\n`;
      }
    ...
    if (config.modems && Array.isArray(config.modems) && config.modems.length > 0) {
      for (const modem of config.modems) {
        ...
        const modemType = modem.type?.toLowerCase() || 'ec25';
        confContent += generateModemSection(modemName, dataPort, modemType, {
      ...
      for (const [modemId, modemConfig] of Object.entries(modemsToGen)) {
        ...
        const modemType = modemConfig.modemType || 'ec25';
        confContent += generateModemSection(modemName, dataPort, modemType, {
          audioPort: modemConfig.audioPort,
          imsi: modemConfig.imsi,
          phoneNumber: modemConfig.phoneNumber,
        });
      }
    }
